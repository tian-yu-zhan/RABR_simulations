...
setwd("~/RAR_un/")
# setwd("~/code_SIM_sharing/simulation_continuous_endpoints/")
# setwd("C:/Users/ZHANTX/Documents/TZ/collaborations/clinical studies/immunology/RZB HS/Cui2007/RAR_unweighted/code/code_SIM_sharing/simulation_continuous_endpoints/")
...
sim.NRAR.func = function(NRAR.input){
  ...
  # summary.table = data.frame("mean" = paste(resp.vec,collapse = ", "))  
  ...
  pred = foreach(itt = 1:n.itt) %dopar% {
    ...
    for (i in 1:n.look){
      ...
      # stand.mean.vec = sapply(2:n.arm, function(x){mean(current.sample.list[[x]])/
      #                                             sd(current.sample.list[[x]])})
      
      # new.sample.n = c(block[1], 
    ...
    ## closure principle
    # closure.func.fit = closure.pool.func(current.sample.list,n.active.arm,
    #                                      one.sided.alpha)
    # RAR.dec.adj = closure.func.fit$closure.dec.final.vec
    ...
    # ASN_trt_vec[-RAR.selected.arm] = NA
    ...
    # RAR.dec.adj.select = RAR.dec.adj[order(dunnett.fit, decreasing = FALSE)]
    ...
    # rand.ratio.mat.add = cbind(n.initial.m.vec/m.total, rand.ratio.mat)
    # rand.ratio.mat.add = rand.ratio.mat
    ...
    # ASN_s = ASN_trt_vec[RAR.selected.arm]
    
    
    
    ## ASN for selected
    # ASN_pbo = length(unlist(current.sample.list[[1]]))
    # ASN_trt_vec = rep(NA, n.active.arm)
    # ASN_s = NA
    
    # RAR.dec.unadj = RAR.p.unadj.func(current.sample.list, n.active.arm, one.sided.alpha)
    # RAR.selected.arm = which.max(unlist(lapply(current.sample.list, function(x){mean(x)/sd(x)*sqrt(length(x))})[-1]))
    # 
    # RAR.dec.selected = RAR.dec.adj[RAR.selected.arm]
    # RAR.ASN.selected = length(unlist(current.sample.list[[RAR.selected.arm+1]]))
    # 
    # if (RAR.dec.selected==1){
    #   ASN_s = ASN_trt_vec[RAR.selected.arm] = RAR.ASN.selected
    # }
    ...
    # p.mat.RAR[itt, as.vector(paste0("reject_unadj_", 1:n.active.arm))] = RAR.p.unadj
    # RAR.p.dunnett.vec = RAR.dunnett.func(current.sample.list, n.arm)
    # RAR.selected.arm = which.min(RAR.p.dunnett.vec)
    # 
    # RAR.dec.selected = as.numeric(RAR.p.dunnett.vec[RAR.selected.arm]<=one.sided.alpha)
    # RAR.ASN.selected = length(unlist(current.sample.list[[RAR.selected.arm+1]]))
    # 
    # if (RAR.dec.selected==1){
    #   p.mat.RAR[itt, "ASN_s"] = p.mat.RAR[itt, c(as.vector(paste0("ASN_", RAR.selected.arm)))] =
    #     RAR.ASN.selected
    # }
    # 
    # p.mat.RAR[itt, "decision"] = RAR.dec.selected
    # p.mat.RAR[itt, paste0("reject_", RAR.selected.arm)] = p.mat.RAR[itt, "decision"]
    # p.mat.RAR[itt, paste0("selected_dose_", RAR.selected.arm)] = 1
  ...
  for (itt in 1:n.itt){
    ...
    # p.mat.RAR[itt, (paste0("reject_component_", closure.name.vec))] = 
    #   pred.temp$reject_closure_component
...
sim.TRAR.func = function(TRAR.input){
  ...
  pred = foreach(itt = 1:n.itt) %dopar% {
    ...
    #######################################################################################
    ## RAR with dunnett test
    # current.sample.list = lapply(1:n.arm, function(x){rnorm(m.total/n.arm, resp.vec[x], 1)})
    
    rand.initial.m.vec = c(r0.RAR, rep((1-r0.RAR)/n.active.arm, n.active.arm))
    # rand.initial.m.vec = rep(1/n.arm, n.arm)
    
    # rand.initial.sample.ind = TRUE
    # while(rand.initial.sample.ind){
    #   n.initial.m.vec = round(as.vector(rmultinom(1, size = m.total, prob = rand.initial.m.vec)))
    #   if (min(n.initial.m.vec)>=2) rand.initial.sample.ind = FALSE
    # } 
    ...
    # n.initial.vec = c(100, 100, 100,700)
    # current.sample.list = lapply(1:n.arm, function(x){rnorm(n.initial.vec[x], resp.vec[x], 1)})
    ...
    for (i in 1:n.look){
      ...
      # tempy.vec = sapply(current.sample.list, function(t){pnorm((mean(t)-
      #                   mean(current.sample.list[[1]]))/sd(t))})
      ...
      # rand.ratio.vec = c(r0.RAR, (1-r0.RAR)*rand.factor.vec/sum(rand.factor.vec))
      
      if (i==n.look) h.current = h.last
      new.sample.n = as.vector(rmultinom(1, size = h.current, prob = rand.ratio.vec))
      
      # print(paste(l.mean.temp, ",", h.mean.temp))
      # print(rand.vec)
      ...
      rand.ratio.mat[, i] = sapply(current.sample.list, length)/
        sum(sapply(current.sample.list, length))
      rand.size.mat[, i] = sapply(current.sample.list, length)
      # rand.ratio.mat[, i] = rand.ratio.vec
    }
    ...
    newlist = list("ASN_pbo" = ASN_pbo,
                   ...
                   "rand_ratio_mat" = rand.ratio.mat.order, 
                   "rand_size_mat" = rand.size.mat.order, 
                   # "RAR.dec.selected" = RAR.dec.selected,
                   # "RAR.ASN.selected" = RAR.ASN.selected,
                   "lim_prob" = lim_prob
    )
    return(newlist)
  }
